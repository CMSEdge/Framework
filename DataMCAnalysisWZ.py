...
def makePlot(lumi, lumi_str, treeDA, treeMC, var, name, nbin, xmin, xmax, theCut, cuts,labelx, logx, returnplot = False, scaleToData = False, normalized = False, cumulative = False, onlyMC = False):
    ...
    theCutLep1W = cuts.AddList([theCut, cuts.lep1W])
    theCutLep2W = cuts.AddList([theCut, cuts.lep2W])
    theCutLep1Z = cuts.AddList([theCut, cuts.lep1Z])
    theCutLep2Z = cuts.AddList([theCut, cuts.lep2Z])
    MCLep1W  = treeMC.getTH1F(lumi, "hMCS_Lep1W%s"%(name), var, nbin, xmin, xmax, theCutLep1W,  "", labelx)
    MCLep2W  = treeMC.getTH1F(lumi, "hMCS_Lep2W%s"%(name), var, nbin, xmin, xmax, theCutLep2W,  "", labelx)
    MCLep1Z  = treeMC.getTH1F(lumi, "hMCS_Lep1Z%s"%(name), var, nbin, xmin, xmax, theCutLep1Z,  "", labelx)
    MCLep2Z  = treeMC.getTH1F(lumi, "hMCS_Lep2Z%s"%(name), var, nbin, xmin, xmax, theCutLep2Z,  "", labelx)

    MCLep1Z.SetName('Lep1 from Z')
    #MCLep1Z  .SetFillColor(r.kOrange)
    MCLep1Z  .SetLineColor(r.kOrange-3)
    MCLep2Z.SetName('Lep2 from Z')
    #MCLep2Z  .SetFillColor(r.kOrange+6)
    MCLep2Z  .SetLineColor(r.kRed-3)
    MCLep1W.SetName('Lep1 from W')
    #MCLep1W  .SetFillColor(r.kGreen)
    MCLep1W  .SetLineColor(r.kGreen-3)
    MCLep2W.SetName('Lep2 from W')
    MCLep2W  .SetLineColor(r.kCyan-3)
    #MCLep2W  .SetFillColor(r.kGreen+6)
    
    maxValLep1W = MCLep1W.GetMaximum() #GetBinContent(MC.GetMaximumBin())
    maxValLep2W = MCLep2W.GetMaximum() #GetBinContent(MC.GetMaximumBin())
    maxValLep1Z = MCLep1Z.GetMaximum() #GetBinContent(MC.GetMaximumBin())
    maxValLep2Z = MCLep2Z.GetMaximum() #GetBinContent(MC.GetMaximumBin())
    maxVal = max(maxValLep1Z, maxValLep2Z)
    MCLep1W.GetYaxis().SetRangeUser(0.1, 2*maxVal)
    MCLep2W.GetYaxis().SetRangeUser(0.1, 2*maxVal)
    MCLep1Z.GetYaxis().SetRangeUser(0.1, 2*maxVal)
    MCLep2Z.GetYaxis().SetRangeUser(0.1, 2*maxVal)
    
    
    print "MCLep1W", MCLep1W.Integral()
    print "MCLep2W", MCLep2W.Integral()
    print "MCLep1Z", MCLep1Z.Integral()
    print "MCLep2Z", MCLep2Z.Integral()

    plot = Canvas.Canvas('DataMC/%s/plot_%s'%(lumi_str,name), 'png,pdf,root', 0.7, 0.4, 0.9, 0.9)
    plot.addHisto(MCLep1Z, "HIST", MCLep1Z.GetName(),'PL',  MCLep1Z.GetLineColor(), 1, 0)
    plot.addHisto(MCLep2Z, "HIST, SAME", MCLep2Z.GetName(),'PL',  MCLep2Z.GetLineColor(), 1, 0)
    plot.addHisto(MCLep2W, "HIST, SAME", MCLep2W.GetName(),'PL',  MCLep2W.GetLineColor(), 1, 0)
    plot.addHisto(MCLep1W, "HIST, SAME", MCLep1W.GetName(),'PL',  MCLep1W.GetLineColor(), 1, 0)
    plot.saveRatio(1, 1, logx, lumi, MCLep1W, MCLep1W)
...
if __name__ == "__main__":
    ...
    makePlot(lumi, lumi_str, treeMC, treeMC, "Lep1_pt_Edge", "lep1_pt_"+jet, 30, 0, 300, cuts.AddList([cuts.goodLepton, cuts.slep, cuts.ThirdLeptonVeto]), cuts, "lep1 p_{T} [GeV] ("+ jet+" jet)",  0, True)
    makePlot(lumi, lumi_str, treeMC, treeMC, "Lep1_eta_Edge", "lep1_pt_"+jet, 20, -5, 5, cuts.AddList([cuts.goodLepton, cuts.slep, cuts.ThirdLeptonVeto]), cuts, "lep1 #eta ("+ jet+" jet)",  0, True)
    makePlot(lumi, lumi_str, treeMC, treeMC, "Lep2_pt_Edge", "lep2_pt_"+jet, 30, 0, 300, cuts.AddList([cuts.goodLepton, cuts.slep, cuts.ThirdLeptonVeto]), cuts, "lep2 p_{T} [GeV] ("+ jet+" jet)",  0, True)
    makePlot(lumi, lumi_str, treeMC, treeMC, "Lep2_eta_Edge", "lep2_eta_"+jet, 20, -5, 5, cuts.AddList([cuts.goodLepton, cuts.slep, cuts.ThirdLeptonVeto]), cuts, "lep2 #eta [GeV] ("+ jet+" jet)",  0, True)
    makePlot(lumi, lumi_str, treeMC, treeMC, "Lep1_pt_Edge", "lep1_pt_"+jet, 30, 0, 300, cuts.AddList([cuts.goodLepton, cuts.slep, cuts.ThirdLeptonVeto]), cuts, "lep1 p_{T} [GeV] ("+ jet+" jet)",  0, True)
